...
尚未配置真实腾讯云凭证；不能把模拟接口测试视为真实云端识别验收。

公网 HTTPS 页面使用 `/api/voice/status` 和 `/api/voice/transcribe`，不要求
WorkBuddy、Node、Python 或本地语音模型。只有旧的 `127.0.0.1:43117` 且含
工作区标记的入口继续使用原 `/voice/*`；不会在云端失败时悄悄改用本地模型。
单文件 HTML 没有 API 后端，语音入口提示打开 HTTPS 站点，仍可直接输入文字。
