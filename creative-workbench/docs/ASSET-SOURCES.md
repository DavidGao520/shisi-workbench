...
- 辣子鸡、青椒茄子插画用 Codex 内置 ImageGen 原创生成，为透明底食物主体，页面另配原作青花盘。两张原始 PNG 均为 1254×1254，保留 alpha 转为 WebP，未改绘。青椒茄子首次生成因网络错误失败，用户再次要求补齐后已生成并替换临时实拍主图；真实照片只保留在实做记录与缩略图中。两张插画的文件路径、Git blob SHA-1 及完整提示词见 [本次生成记录](DISH-ART-PROMPTS-2026-09-10.md)。
