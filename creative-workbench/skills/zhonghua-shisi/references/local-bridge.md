...
网页语音通道 `/voice/status` / `/voice/transcribe` 受相同本机来源和请求头检查保护，固定转交至独立腾讯云 SCF 的 `https://1482519311-ite8vah0bd.ap-guangzhou.tencentscf.com/api/voice/status` / `api/voice/transcribe`，不依赖旧的 Sites 入口。本机程序不持有腾讯密钥、不调用本机模型、不读取库存、不使用照片 ticket。浏览器转换为 16 kHz 单声道 PCM16 WAV，最多 1,920,044 字节 / 60 秒；中继按固定配置将音频编码为 JSON/Base64 后发往 SCF。最多一个中继转写，超时或取消中止，无自动重试、无重定向、音频不落盘。状态检查也会随页面断开或停止而取消。
